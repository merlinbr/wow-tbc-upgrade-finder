...
- **Item level:** a readable numeric badge at the top-left of the equipment icon, not the item ID or content phase. Preserve room for gems along the bottom. Item level exists in bundled `UIItem.ilvl` but is not currently exposed by `GearSlotData`; add it from the effective equipped-item data rather than infer it from phase.
