...
<section class="panel report-panel" aria-labelledby="report-heading" data-region="report-view">
  ...
  {#if report.character}
    <p class="report-character">Character: {report.character.name || 'Unnamed'} · Level 70 {report.character.race || 'Unknown race'} {report.character.class || 'Unknown class'}{report.character.spec ? ` · ${report.character.spec}` : ''} · Phase {report.character.phase ?? '—'}</p>
  {/if}
